# -*- coding: utf-8 -*-
"""
Advisor OSC Finance v1.6.3 테스트

파이프라인:
- Document Parser: GPT-4o Vision
...
# v1.6.3 설정
VERSION = "v1.6.3"
COLLECTION_NAME = "advisor_osc_finance_v163_gpt4o_vision"  # GPT-4o Vision + OpenAI Embedding
...
async def process_single_case(item, retriever, reranker, config, semaphore, progress_counter, total):
    async with semaphore:
        ...
        try:
            no_rerank_config = RetrievalConfig(top_k=config.rerank_top_k, rerank=False, expand_to_parent=config.expand_to_parent, use_hybrid=config.use_hybrid)
...
async def run_test():
    ...
    output_file = output_dir / f"finance_{VERSION}_gemini_ocr_openai_bge_{timestamp}.json"

    output_data = {"version": VERSION, "parser": "gemini_ocr", "embedding_model": "text-embedding-3-large",
                   "reranker": "bge-reranker-v2-m3", "llm_model": "gpt-5-mini", "collection": COLLECTION_NAME,
                   "timestamp": timestamp, "total": total, "correct": total_correct, "accuracy": overall_acc,
                   "by_context": {ct: {"correct": s["correct"], "total": s["total"],
                                       "accuracy": s["correct"]/s["total"]*100 if s["total"] > 0 else 0}
                                  for ct, s in by_context.items()}, "results": results}
