# -*- coding: utf-8 -*-
"""
Advisor OSC Finance v1.6.2 테스트
...
# v1.6.2 설정
VERSION = "v1.6.2"
COLLECTION_NAME = "advisor_osc_finance_pymupdf"  # 기존 PyMuPDF+OpenAI 컬렉션 재사용
OPENAI_API_KEY = os.getenv("OPENAI_API_KEY")

# BGE Reranker 설정 (GPU 1번 사용 - Docker는 GPU 0 사용 중)
os.environ["CUDA_VISIBLE_DEVICES"] = "1"
os.environ["BGE_DEVICE"] = "cuda"
os.environ["BGE_USE_FP16"] = "false"  # FP16 비활성화로 Float/Half 오류 방지
...
class BGERerankerWrapper:
    """BGE Reranker Wrapper for HierarchicalRetriever with Thread-safe Lock"""

    def __init__(self):
        self._model = None
        self._initialized = False
        self._lock = threading.Lock()  # Thread-safe Lock 추가

    def initialize(self) -> bool:
        """BGE Reranker 초기화 (FlagEmbedding 1.3+)"""
        ...
        try:
            ...
            # CUDA_VISIBLE_DEVICES=1 이므로 cuda:0으로 지정 (실제로 GPU 1 사용)
    ...
    def rerank(self, query: str, documents: list[str], top_k: int = 5) -> list[tuple[str, float]]:
        ...
        try:
            ...
            # Thread-safe Lock으로 동시 접근 방지 (Already borrowed 오류 해결)
            with self._lock:
    ...
    async def rerank_async(self, query: str, documents: list[str], top_k: int = 5) -> list[tuple[str, float]]:
        """비동기 리랭킹 (실제로는 동기 실행)"""
...
async def process_single_case(item, retriever, reranker, config, semaphore, progress_counter, total):
    ...
    async with semaphore:
        ...
        try:
            # 검색 수행 (rerank=False로 설정, 별도 BGE rerank 수행)
            no_rerank_config = RetrievalConfig(
                top_k=config.rerank_top_k,  # 많이 가져와서 rerank
                rerank=False,
...
async def run_test():
    ...
    print(f"  Reranker: BGE v2-m3 (GPU)")
    print(f"  LLM: GPT-5-mini")
    print(f"  Judge: GPT-5-mini")
    ...
    try:
        ...
        if COLLECTION_NAME not in collection_names:
            ...
            print(f"   먼저 인덱싱을 실행하세요: python index_finance_v162_pymupdf_openai.py")
            return 0
    ...
    config = RetrievalConfig(
        top_k=15,
        rerank_top_k=50,
        rerank=False,  # BGE reranker 별도 사용
    ...
    tasks = [
        process_single_case(item, retriever, reranker, config, semaphore, progress_counter, total)
    ...
    output_file = output_dir / f"finance_{VERSION}_openai_bge_{timestamp}.json"

    output_data = {
        "version": f"Advisor OSC {VERSION} (PyMuPDF + OpenAI + BGE + GPT-5-mini)",
        "parser": "pymupdf",
