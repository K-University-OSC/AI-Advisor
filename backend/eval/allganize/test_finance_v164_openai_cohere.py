...
파이프라인 (OpenAI 권장 구성):
- Document Parser: PyMuPDF4LLM (텍스트) + GPT-4o (이미지 캡션)
- Embedding: OpenAI text-embedding-3-large (3072 dim)
- Reranker: Cohere Rerank-v3 (BGE 대신 안정적인 Cohere 사용)
- LLM: GPT-5-mini
- Judge: GPT-5-mini

Collection: advisor_osc_finance_pymupdf (기존 컬렉션 재사용)
"""
...
# v1.6.4 설정
VERSION = "v1.6.4"
COLLECTION_NAME = "advisor_osc_finance_pymupdf"  # PyMuPDF + OpenAI Embedding 컬렉션
...
class CohereRerankerWrapper:
    """Cohere Reranker Wrapper - 안정적인 클라우드 기반 리랭커"""
    ...
    def initialize(self) -> bool:
        ...
        try:
            ...
            print("Cohere Reranker 초기화 완료 (rerank-v3.5)")
            return True
        except Exception as e:
            print(f"Cohere Reranker 초기화 실패: {e}")
            return False
    ...
    def rerank(self, query: str, documents: list[str], top_k: int = 15) -> list[tuple[str, float]]:
        ...
        try:
            ...
            for item in response.results:
                doc_text = item.document.text if hasattr(item.document, 'text') else documents[item.index]
                results.append((doc_text, item.relevance_score))
...
async def process_single_case(item, retriever, reranker, config, semaphore, progress_counter, total):
    async with semaphore:
        ...
        try:
            # 검색 수행 (rerank=False, 별도 Cohere rerank)
...
async def run_test():
    ...
    print(f"  파서: PyMuPDF4LLM + GPT-4o ImageCaptioning")
    print(f"  임베딩: OpenAI text-embedding-3-large")
    print(f"  Reranker: Cohere rerank-v3.5")
    ...
    # Cohere Reranker 초기화
    reranker = CohereRerankerWrapper(api_key=COHERE_API_KEY)
    if not reranker.initialize():
        print("Cohere Reranker 초기화 실패 - Reranking 없이 진행")
    ...
    output_file = output_dir / f"finance_{VERSION}_openai_cohere_{timestamp}.json"
