# -*- coding: utf-8 -*-
"""
Advisor OSC Finance v1.6.6 테스트 (이미지 캡셔닝 추가)

파이프라인:
- Document Parser: PyMuPDF4LLM
- Image Captioning: GPT-4o Vision (추가!)
...
Collection: advisor_osc_finance_v166_with_caption
"""
...
# v1.6.6 설정
VERSION = "v1.6.6"
COLLECTION_NAME = "advisor_osc_finance_v166_with_caption"
...
6. 이미지 설명(IMAGE CAPTION)에서 관련 정보를 찾으면 반드시 포함하세요
...
# ============================================================================
# GPT-4o Vision Image Captioner
# ============================================================================

class GPT4oImageCaptioner:
    """GPT-4o Vision 기반 이미지 캡셔닝"""
                ...
                try:
                    ...
                    response = await self.client.chat.completions.create(
                        ...
                        messages=[{
                            "role": "user",
                            "content": [
                                {"type": "text", "text": "이 이미지를 한국어로 상세히 설명해주세요. 차트나 그래프인 경우 데이터와 트렌드를 포함해주세요. 테이블인 경우 주요 수치를 포함해주세요."},
...
async def index_documents():
    """문서 인덱싱 (PyMuPDF4LLM + GPT-4o Image Captioning + OpenAI Embedding)"""
    print("=" * 60)
    print(f"v1.6.6 인덱싱 시작 (이미지 캡셔닝 포함)")
    print("=" * 60)
    ...
    for i, pdf_path in enumerate(pdf_files, 1):
        ...
        try:
            ...
            # GPT-4o Vision으로 이미지 캡셔닝
            captions = await captioner.caption_images(pdf_path)
            print(f"  - 페이지: {total_pages}, 이미지 캡션: {len(captions)}개")
            ...
            if captions:
                caption_text = "\n\n## IMAGE CAPTIONS\n\n"
                for cap in captions:
                    caption_text += f"### [Page {cap['page']}, Image {cap['image_idx']}]\n{cap['caption']}\n\n"
                md_text += caption_text
            ...
            elements = [ParsedElement(
                ...
                metadata={"parser": "pymupdf4llm+gpt4o", "source": pdf_path.name, "captions": len(captions)}
            )]
            ...
            parsed_doc = ParsedDocument(
                ...
                elements=elements,
                metadata={"parser": "pymupdf4llm+gpt4o", "captions": len(captions)}
            )
...
async def run_test():
    ...
    print(f"  파서: PyMuPDF4LLM + GPT-4o Vision (이미지)")
    ...
    output_file = output_dir / f"finance_{VERSION}_with_image_caption_{timestamp}.json"
